...
var createJoinSpaceMsgs = function (space, addUsers) {
    ...
    User.findOne({_id: mongoose.Types.ObjectId(space.userId)}, function (err, doc) {
        ...
        for (var i in addUsers) {
            ...
            temp = temp.replace('{content}', user.userName + ' invite you join ' + space.spaceName)
...
var createLeaveSpaceMsgs = function (space, removeUsers) {
    ...
    User.findOne({_id: mongoose.Types.ObjectId(space.userId)}, function (err, doc) {
        ...
        for (var i in removeUsers) {
            ...
            temp = temp.replace('{content}', user.userName + ' remove you from ' + space.spaceName);
            msg.content = temp;
            msg.createDate = Date.now();
            msg.valid = true;
            msgs.push(msg);
        ...
        if (!err) {
            ...
            if (found) {
                User.findOne({_id: mongoose.Types.ObjectId(space.userId)}, function (err, doc) {
                    ...
                    msg.from = space.userId;
                    var temp = TEMPLATE.MESSAGE.LEAVE_SPACE;
                    temp = temp.replace('{content}', owner.userName + ' remove you from ' + space.spaceName);
                    msg.content = temp;
                    msg.createDate = Date.now();
                    msg.valid = true;
        ...
        if (!err) {
            ...
            if (!found) {
                User.findOne({_id: mongoose.Types.ObjectId(space.userId)}, function (err, doc) {
                    ...
                    msg.from = space.userId;
                    var temp = TEMPLATE.MESSAGE.JOIN_SPACE;
                    temp = temp.replace('{content}', owner.userName + ' invite you join ' + space.spaceName)
                        .replace('{userId}', msg.to).replace('{spaceId}', space._id);
                    msg.content = temp;
                    msg.createDate = Date.now();
                    msg.valid = true;
